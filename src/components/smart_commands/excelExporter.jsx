...
const formatCell = (val) => {
  ...
  if (typeof val === 'object') {
    if (val['رقم_اللوحة_عربي']) {
      return `لوحة: ${val['رقم_اللوحة_عربي']} | حالة: ${val['حالة_السيارة'] || '-'}`;
    }
...
export async function exportToExcel({ title, entity, fields, results }) {
  ...
  const sheet = workbook.addWorksheet(title.substring(0, 30) || 'تقرير', {
    views: [{ rightToLeft: true, state: 'frozen', ySplit: 3 }],
    properties: { defaultRowHeight: 22 },
  });

  // عنوان الرئيسي مدموج
  const totalColumns = fields.length + 1; // +1 للرقم التسلسلي
  ...
  infoCell.value = `عدد السجلات: ${results.length} | تاريخ التقرير: ${new Date().toLocaleDateString('ar-SA')}`;
  ...
  sheet.headerFooter.oddFooter = `&L&"Cairo"&10تم الإنشاء: ${new Date().toLocaleDateString('ar-SA')}&R&"Cairo"&10صفحة &P من &N`;
